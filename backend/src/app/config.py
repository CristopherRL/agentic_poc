...
# Define session TTL for conversation memory
def _default_session_ttl_seconds() -> int:
    """
    Time-to-live for conversation sessions in seconds.
    Sessions expire after this duration and are cleaned up automatically.
...
def _default_sql_generation_system_prompt() -> str:
    return (
        ...
        "CRITICAL: When conversation history is provided, maintain context from previous questions. If a previous question filtered by brand/model/year/etc., "
        "and the current question only changes one aspect (e.g., 'what about 2023'), combine the filters from both questions."
    )
...
# Define required data tables for SQL queries
def _default_required_data_tables() -> list[str]:
    """
    List of required data tables in SQLite database.
    These tables are checked during startup to determine if SQL ingestion is needed.
    """
    return [
        "DIM_MODEL",
        "DIM_COUNTRY",
...
def _default_sql_generation_user_prompt() -> str:
    return (
        ...
        "2. **Maintain Conversation Context:** If conversation history is provided, extract relevant filters from previous questions and combine them with the current question's filters. For example:\n"
        "   - Previous: 'Toyota sales in 2022' → filters: brand='Toyota', year=2022\n"
        "   - Current: 'what about 2023' → combine: brand='Toyota' (from previous) + year=2023 (from current)\n"
        "   - Current: 'but only Toyota' → combine: brand='Toyota' (from current) + year=2023 (from previous)\n"
        "3. **Use Exact Values:** You MUST use the exact filter values from the question or conversation context (e.g., `fs.year = 2024` for '2024').\n"
        "4. **NO INVENTED FILTERS:** NEVER add filters that are not in the question or conversation context. **NEVER** use 'Sample rows' for filter values.\n"
        "5. **Handle Comparisons:** For questions like 'Compare Toyota vs Lexus', use an `IN` clause (e.g., `dm.brand IN ('Toyota', 'Lexus')`) and `GROUP BY` the comparison column (e.g., `GROUP BY dm.brand`).\n"
        "6. **Handle Regions:** For regional questions (e.g., 'Western Europe'), you MUST `JOIN DIM_COUNTRY dc ON fs.country_code = dc.country_code` and filter on `dc.region`.\n"
...
class Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_file=".env",  # Backend .env file (relative to backend/ directory)
    ...
    openai_api_key: str = Field(
        default="test-key-for-tests-only",
        validation_alias=AliasChoices("openai_api_key", "open_api_key"),
        description="OpenAI API key. Required in production, defaults to test value for testing."
    )
    ...
    required_data_tables: list[str] = Field(default_factory=_default_required_data_tables)
    ...
    session_ttl_seconds: int = Field(default_factory=_default_session_ttl_seconds)
